...
class FiveQueensQuerySolutionDemo extends NQueensQuerySolutionDemo {
	...
	public void inferNQueensSolutionNum() {
		...
		while (it.hasNext()) {
			...
			messageBoxController.setMessageText("五皇后問題共有" + count + "組解");
		}
	}
	public void NQueensLastSolutionDemo(int grids) {
		...
		if (demoCount < getFiveQueensSolutionCounts()) { // flag 0 ~ 9最多共10次
			flagCountMinus();
			if (demoCount > -1) {
				...
				messageBoxController.setMessageText("五皇后問題共有"+getFiveQueensSolutionCounts()+"組解 目前是第" + (demoCount + 1) + "種解");
	...
	public void NQueensNextSolutionDemo(int grids) {
		...
		/*
		 * if (demoCount == 10) { // if(flag == 10)被寫死了 應該要能跟算出來的N皇后問題解數動態連結
		 * leftControlPanel.disabledShowNextAnswer(); // 到了最後一組解 //
		 * 就不再按"下一組解"這個按鈕了 cleardemoCount(); }
		 */
		disabledShowAnswerButton(getFiveQueensSolutionCounts()-1);
	}
