...
class EightQueensQuerySolutionDemo extends NQueensQuerySolutionDemo {
	...
	public void inferNQueensSolutionNum()  {
		...
		while (it.hasNext()) {
			...
			messageBoxController.setMessageText("八皇后問題共有" + count + "組解");
	...
	public void NQueensLastSolutionDemo(int grids) {
		...
		if (demoCount < getEightQueensSolutionCounts()) { // flag 0 ~ 91最多共92次
			flagCountMinus();
			if (demoCount > -1) {
				...
				messageBoxController.setMessageText("八皇后問題共有"+getEightQueensSolutionCounts()+"組解 目前是第" + (demoCount + 1) + "種解");
