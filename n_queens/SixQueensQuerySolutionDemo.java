...
class SixQueensQuerySolutionDemo extends NQueensQuerySolutionDemo {
	...
	public void inferNQueensSolutionNum() {
		...
		while (it.hasNext()) {
			...
			messageBoxController.setMessageText("六皇后問題共有" + count + "組解");
		}
	}
	...
	public void NQueensLastSolutionDemo(int grids) {
		...
		if (demoCount < getSixQueensSolutionCounts()) { // flag 0 ~ 3最多共4次
			flagCountMinus();
			if (demoCount > -1) {
				...
				messageBoxController.setMessageText("六皇后問題共有"+getSixQueensSolutionCounts()+"組解 目前是第" + (demoCount + 1) + "種解");
